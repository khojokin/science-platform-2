...
export async function POST(request: NextRequest) {
  try {
    ...
    const goals = Array.isArray(body.goals) ? body.goals.map((item: unknown) => String(item).trim()).filter(Boolean).slice(0, 12) : [];
    const interests = Array.isArray(body.interests) ? body.interests.map((item: unknown) => String(item).trim()).filter(Boolean).slice(0, 12) : [];
